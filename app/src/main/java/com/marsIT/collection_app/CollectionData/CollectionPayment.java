...
//        }
    ...
    private String buildCategorySms(String prNumber) {
        ...
        for (CategoryItem item : cpDeductionList) {
            ...
            double amount = parseDouble(item.getAmount());
            if (amount <= 0) continue;   // skip empty / zero deductions
        ...
        // NO VALID DEDUCTIONS → DO NOT SEND COLC
        if (index == 0) return "";

        return getColPrefix('C') + "/" +
            prNumber + "/" +
            (cpSelectedCustomerID != null ? cpSelectedCustomerID : "") +
            "/" +
            body.toString();
...
//                        }
                    ...
                    // Skipping COLC if no deduction is added
                    boolean hasValidDeduction = false;

                    for (CategoryItem item : cpDeductionList) {
                        if (item != null && parseDouble(item.getAmount()) > 0) {
                    ...
                    if (hasValidDeduction) {
                        String categoryMsg = buildCategorySms(prNumber);

                        // Extra safety: don't send header-only COLC
                        if (categoryMsg.matches("^COL.*C/.+/.+/$")) {
                            Log.d("SMS_LOG", "COLC has no items. Skipped.");
                        } else {
                            Log.d("SMS_LOG", "Sending COLC to: " + receiver);
                            sendSmsMultipart(receiver, categoryMsg);
                        }
                    } else {
                        Log.d("SMS_LOG", "No deductions found. Skipping COLC.");
                    }
